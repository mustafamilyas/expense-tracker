...
use crate::{error::AppError, repos::category_alias::{CategoryAlias, CategoryAliasRepo, CreateCategoryAliasPayload, UpdateCategoryAliasPayload}, types::AppState, auth::{AuthContext, AuthSource}};
...
#[utoipa::path(post, path = "/categories-aliases", request_body = CreatePayload, responses((status = 200, body = CategoryAlias)), tag = "Category Aliases", operation_id = "createCategoryAlias", security(("bearerAuth" = [])))]
pub async fn create(State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Json(payload): Json<CreatePayload>) -> Result<Json<CategoryAlias>, AppError> {
...
#[utoipa::path(put, path = "/categories-aliases/{alias_uid}", params(("alias_uid" = Uuid, Path)), request_body = UpdatePayload, responses((status = 200, body = CategoryAlias)), tag = "Category Aliases", operation_id = "updateCategoryAlias", security(("bearerAuth" = [])))]
